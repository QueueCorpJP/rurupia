...
const BlogCategory = () => {
  ...
  // Fetch blog posts from Supabase filtered by category
  useEffect(() => {
  ...
  if (isLoading) {
    return (
      <Layout>
        <SEO 
          title={`${decodeURIComponent(category || '')} に関する記事`}
          description={`${decodeURIComponent(category || '')} カテゴリの最新記事一覧。`}
        />
  ...
  return (
    <Layout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl mb-2">
            カテゴリ: {decodeURIComponent(category || '')}
          </h1>
          <p className="text-muted-foreground">
